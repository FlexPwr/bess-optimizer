...
def step1_optimize_daa(n_cycles:int, energy_cap:int, power_cap:int, daa_price_vector:list):
    ...
    def soc_step_constraint(model, i):
        """
    ...
    def charge_cycle_limit(model):
        ...
        """
        return sum(model.cha_daa[i] * power_cap / 4 for i in model.Q) <= volume_limit

    def discharge_cycle_limit(model):
        """
        Sum of all discharges has to be below the daily limit. (Constraint 1.7)
        """
        return sum(model.dis_daa[i] * power_cap / 4 for i in model.Q) <= volume_limit

    def cha_daa_quarters_1_2_parity(model, i):
        """
    ...
    def dis_daa_quarters_2_3_parity(model, i):
        """
        Set daa positions of quarter 2 and 3 of each hour equal. (Constraint 1.9)
        """
        return model.cha_daa[4 * i + 2] == model.cha_daa[4 * i + 3]

    def dis_daa_quarters_3_4_parity(model, i):
        """
...
def step2_optimize_ida(n_cycles:int, energy_cap:int, power_cap:int, ida_price_vector:list, step1_cha_daa:list, step1_dis_daa:list):
    ...
    def charge_cycle_limit(model):
        ...
        """
        return ((np.sum(step1_cha_daa) + sum(model.cha_ida[i] for i in model.Q) - sum(model.dis_ida_close[i] for i in model.Q)) * power_cap/4 <= volume_limit)
    ...
    def discharge_cycle_limit(model):
        ...
        """
        return ((np.sum(step1_dis_daa) + sum(model.dis_ida[i] for i in model.Q) - sum(model.cha_ida_close[i] for i in model.Q)) * power_cap/4 <= volume_limit)


    def cha_close_logic(model, i):
        """
    ...
    def charge_rate_limit(model, i):
        """
         Sum of cha_ida[i] and cha_daa[i] has to be less or equal to 1. (Constraint 2.10)
         """
        return model.cha_ida[i] + step1_cha_daa[i-1] <= 1
   

    def discharge_rate_limit(model, i):
        """
         Sum of dis_ida[i] and dis_daa[i] has to be less or equal to 1. (Constraint 2.11)
         """
...
def step3_optimize_idc(n_cycles:int, energy_cap:int, power_cap:int, idc_price_vector:list, step2_cha_daaida:list, step2_dis_daaida:list):
    ...
    def charge_cycle_limit(model):
        ...
        """
        return (np.sum(step2_dis_daaida) + sum(model.dis_idc[i] for i in model.Q) - sum(model.cha_idc_close[i] for i in model.Q)) * power_cap/4 <= volume_limit
    ...
    def discharge_cycle_limit(model):
        ...
        """
        return (np.sum(step2_cha_daaida) + sum(model.cha_idc[i] for i in model.Q) - sum(model.dis_idc_close[i] for i in model.Q)) * power_cap/4 <= volume_limit
    ...
    def charge_rate_limit(model, i):
        """
         Sum of cha_idc[i] and cha_daaida[i] has to be less or equal to 1. (Constraint 3.10)
         """
    ...
    def discharge_rate_limit(model, i):
        """
         Sum of dis_idc[i] and dis_daaida[i] has to be less or equal to 1. (Constraint 3.11)
         """
